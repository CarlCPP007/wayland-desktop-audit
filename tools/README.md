...
| tool | what it does |
|---|---|
| `pkg-closure-v2.py` | the resolver. Downloads the Arch databases into `data/db/` on first run, then works offline. Resolves every stack's full transitive closure, records the parent chain for each package, and writes `<out>.json`, `<out>.tsv` and `<out>-guesses.json`. The JSON carries `db_vintage` — a `sha256`, `mtime` and byte count per database — so a difference between two runs can be attributed to upstream drift instead of guessed at. A stack root that is not a package name but is provided by exactly one package (ricelin's `swww`, which `awww` both provides and replaces) is resolved to that provider and recorded in `provider_substitutions`; an ambiguous provider is left unresolved on purpose, because that choice belongs to a human. |
...
| `verify-reproducibility.sh` | the gate. Clones the public repository, runs every stage with no setup, then tests two claims separately: **determinism** (same database snapshot → byte-identical output) and **drift** (a later snapshot → same membership, moved sizes, attributed via `db_vintage`). Exit code is non-zero only for a real defect. |
